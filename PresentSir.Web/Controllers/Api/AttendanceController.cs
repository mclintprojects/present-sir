...
namespace PresentSir.Web.Controllers.Api
{
    public class AttendanceController : ApiController
    {
        public HttpResponseMessage Get(int pageNumber, int perPage, int classId, string date)
        {
            ...
            var attendance = ApplicationDbContext.Instance.Attendance.Include(x => x.Student).Include(x => x.Student.User).Find(x => x.ClassId == classId && x.ClassDate.ToShortDateString() == dateTime.ToShortDateString()).ToPagedList(pageNumber, perPage);
